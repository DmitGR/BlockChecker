	...
	public partial class MainForm : Form
	{
		...
		void Work()
		{
			...
			try
			{
				...
				SessionCotroller.ReadLastRecord(SessionCotroller.Session);

				using (StreamReader sr = new StreamReader(filePath, System.Text.Encoding.Default))
				{
					string line;
					string[] temp;
					double prevtime = SessionCotroller.Session.LastRecord / 5;

					for (completed = SessionCotroller.Session.LastRecord; completed < totalLines; completed++)
					{
					...
					//	completed++;
						...
						double calc = (sw_total.Elapsed.TotalSeconds + prevtime) / completed * totalLines;
						UpdateTimer(calc - sw_total.Elapsed.TotalSeconds + prevtime);
					}
			...
			ProgressBarChanged(progressBar.Maximum);
			BtnSwitch();
		...
		private void SessionList_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			{
				SessionCotroller.Session = (Session)SessionList.SelectedItem;
				sessionFile.Text = SessionCotroller.Session.FilePath;

			//filePathLabel.Text = sessionFile.Text;
			//filePath = sessionFile.Text;
			}

			newSessionBtn.Checked = false;
		...
		private void sessionPage_Enter(object sender, EventArgs e)
		{
			LoadSession();
